    ...
    async fn test_delete_outdated_posts() {
        ...
        sqlx::query(
            "INSERT INTO posts (title, created, updated, link, author, avatar, rule, createdAt) 
             VALUES ('过期帖子', '2020-01-01', DATE_SUB(CURDATE(), INTERVAL 10 DAY), 'https://example.com/old', '作者', 'avatar.jpg', 'test', '2020-01-01')"
        )
        .execute(&pool)
        .await
        .unwrap();
        ...
        sqlx::query(
            "INSERT INTO posts (title, created, updated, link, author, avatar, rule, createdAt) 
             VALUES ('新帖子', '2023-01-01', CURDATE(), 'https://example.com/new', '作者', 'avatar.jpg', 'test', '2023-01-01')"
        )
        .execute(&pool)
        .await
        .unwrap();
